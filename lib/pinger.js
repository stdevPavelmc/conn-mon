...
export const Pinger = GObject.registerClass({
    ...
    _executePing() {
        ...
        try {
            ...
            this._pendingSubprocess = launcher.spawnv(cmd);
            
            // Wait for subprocess to complete
            this._pendingSubprocess.communicate_async(null, null, (subprocess, res) => {
                try {
                    const [, stdout_bytes] = subprocess.communicate_finish(res);
                    ...
                    if (success) {
                        ...
                        const output = stdout_bytes
                            ? new TextDecoder().decode(stdout_bytes.toArray())
                            : '';
