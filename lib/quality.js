...
export const PROFILES = {
    auto: {
        ...
        maxLatency: 100,  // Default fallback
        minAckPercent: 95
    },
    lan: {
        id: 'lan',
...
export class QualityCalculator {
     ...
     * 
     * Formula: Quality = (PacketScore * 0.6) + (LatencyScore * 0.4)
     * 
    ...
    calculateQuality(packetLossPercent, avgLatencyMs) {
        ...
        // Packet score: 100% ack = 100 points, 0% ack = 0 points
        const packetAckPercent = 100 - packetLossPercent;
        const packetScore = packetAckPercent;
        
        // Latency score: 0ms = 100 points, maxLatency or more = 0 points
        // Linear degradation from 0 to maxLatency
        let latencyScore = 100;
        if (avgLatencyMs > 0) {
            latencyScore = Math.max(0, 100 - ((avgLatencyMs / profile.maxLatency) * 100));
        }
    ...
    detectProfile(avgLatencyMs) {
        ...
        if (latency <= PROFILES.lan.maxLatency) {
            bestMatch = PROFILES.lan;
        } else if (latency <= PROFILES.fiber.maxLatency) {
            bestMatch = PROFILES.fiber;
        } else if (latency <= PROFILES['wifi-local'].maxLatency) {
            bestMatch = PROFILES['wifi-local'];
        } else if (latency <= PROFILES['wifi-internet'].maxLatency) {
            bestMatch = PROFILES['wifi-internet'];
        } else if (latency <= PROFILES.adsl.maxLatency) {
